...
def plot_data(TrainingData_labels, TrainingData_features, TestingData_labels, TestingData_features): 
    plt.subplot(1,2,1) 
    plt.plot(TrainingData_features[np.array((TrainingData_labels==0).nonzero())][0,:,0], TrainingData_features[np.array((TrainingData_labels==0).nonzero())][0,:,1], 'g.') 
    plt.plot(TrainingData_features[np.array((TrainingData_labels==1).nonzero())][0,:,0], TrainingData_features[np.array((TrainingData_labels==1).nonzero())][0,:,1], 'r.') 
    plt.title('TrainingData') 
    plt.subplot(1,2,2) 
    plt.plot(TestingData_features[np.array((TestingData_labels==0).nonzero())][0,:,0], TestingData_features[np.array((TestingData_labels==0).nonzero())][0,:,1], 'g.') 
    plt.plot(TestingData_features[np.array((TestingData_labels==1).nonzero())][0,:,0], TestingData_features[np.array((TestingData_labels==1).nonzero())][0,:,1], 'r.') 
    plt.show() 
...
def train_MLP_hyperparams(TrainingData_labels, TrainingData_features): 
    ...
    plt.plot(hyperparam_candidates, hyperparam_performance, 'r.') 
    ...
    plt.ylim([0,1]) 
    plt.plot(hyperparam_candidates[max_perf_index], hyperparam_performance[max_perf_index], 'gx') 
...
def plot_trained_model(model_type, model, features, labels): 
    ...
    plt.plot(features[correct][:,0], features[correct][:,1], 'g.', alpha=0.25) 
